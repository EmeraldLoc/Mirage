...
void CoopNetNetworkSystem::update() {
    ...
    if (needsLobbyCreate && localUserId != 0) {
        ...
        CoopNetRc rc = coopnet_lobby_create("sm64coopdx", gServerConfig.version.c_str(), gServerConfig.name.c_str(), gServerConfig.description.c_str(), 16, gServerConfig.password.c_str(), "");
        if (rc == COOPNET_OK) {
