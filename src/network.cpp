...
void CoopNetNetworkSystem::update() {
    ...
    if (needsLobbyCreate && localUserId != 0) {
        ...
        CoopNetRc rc = coopnet_lobby_create("sm64coopdx", gServerConfig.version.c_str(), gServerConfig.name.c_str(), "horse", 16, gServerConfig.password.c_str(), "peak description right?");
        if (rc == COOPNET_OK) {
